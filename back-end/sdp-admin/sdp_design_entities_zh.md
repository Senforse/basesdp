...
现犼：`sys_user` 含 `deptId`；无角色/权限/菜单表与接口；前端 `hydrateDefaultMenu` 静态菜单。规划中：用户-角色-权限多对多；组织树；种子数据幂等插入。
...
## UTF-8 materialization

Run from repo machine (ASCII-only script; writes UTF-8 without BOM):

`python back-end/sdp-admin/materialize_sdp_design_md.py`

Output: `back-end/sdp-admin/sdp_design_utf8_zh.md`. Optionally rename to `sdp\u8bbe\u8ba1\u65b9\u6848.md` locally.
